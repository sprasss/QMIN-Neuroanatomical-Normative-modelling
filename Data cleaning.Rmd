---
title: "Preparing data for running NM"
output:
...
x <- df_t$"lh_G.S_frontomargin_thickness"
h <- hist(x, breaks=10, col="red", xlab="frontomargin thickness lh",
...
x <- df_t$"lh_G&S_occipital_inf_thickness"
h <- hist(x, breaks=10, col="red", xlab="occipital inf lh",
   main="Histogram with Normal Curve")
xfit<-seq(min(x),max(x),length=40)
yfit<-dnorm(xfit,mean=mean(x),sd=sd(x))
yfit <- yfit*diff(h$mids[1:2])*length(x)
